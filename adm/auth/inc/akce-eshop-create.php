...
$o_picture = $_POST['uploaded'];
if(!empty($_POST['uploaded2'])){$o_picture2 = $_POST['uploaded2'];}else{$o_picture2 = NULL;}
if(!empty($_POST['uploaded3'])){$o_picture3 = $_POST['uploaded3'];}else{$o_picture3 = NULL;}
$o_vel = $_POST['velikost'];
$o_cena = $_POST['cena'];
$o_doporucene = $_POST ['doporucujeme'];
$o_skladem = $_POST ['dodani'];
$o_timestamp = time();
//$directoryPath = "http://".$_SERVER["HTTP_HOST"]."/";
//
...
$result = move_uploaded_file($_FILES['uploaded']['tmp_name'], '/var/www/clients/client0/web8/web/dev/balonparty/imgs-shop/'.$o_timestamp .'-'. $_FILES['uploaded']['name']);
      if ($result) {
       ...
       // $img1 = './../../upload/small/'.$_FILES['photo']['name'];
        ...
        $qry = "INSERT into products2 (`keywords`, `description`, `title`, `www`, `kategorie`, `subkategorie`, `vlastnost`, `nazev`, `text`, `pictures`, `pictures2`, `pictures3`, `velikost`, `cena`, `doporucene`, `skladem`) values ('$o_key', '$o_desc', '$o_title', '$o_www', '$_kategorie', '$_subkategorie', '$_vlastnost', '$o_nazev', '$o_product', '$img1', '$o_picture2', '$o_picture3', '$o_vel', '$o_cena', '$o_doporucene', '$o_skladem')";
