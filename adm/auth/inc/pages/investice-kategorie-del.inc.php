...
//$datum = date("Y-m-j");
?>
              <h1 class="edit-list-title">Smazání kategorie <span>(úprava kategorie)</span></h1>
              
              <div class="edit-list-wrap">

                  <form class="edit-list-form" action="./inc/akce-delete-kategorie.inc.php" method="post">
<?php
...
$vysledek=mysql_query("SELECT * FROM categories WHERE id='$id'");
while ($zaznam=MySQL_Fetch_Array($vysledek)): 
                                echo "<fieldset class=\"edit-form-left\">
                                      <input name=\"id\" type=\"hidden\" value='".$zaznam['id']."'/>
                                      <label for=\"page-title\">Nadpis kategorie:</label> 
                                      <input class=\"text\" type=\"text\" value='".$zaznam['nazev']."' id=\"page-title\" name=\"page-title\" title=\"Název novinky\"/>
                                  </fieldset>
                                  <fieldset class=\"edit-form-right\">
                                      <label for=\"date\">Id kategorie:</label>
                                      <input class=\"text\" type=\"text\" value='".$zaznam['id']."' id=\"date\" name=\"date\" title=\"Datum\"/>
                                  </fieldset>
